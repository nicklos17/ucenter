...
define(function(require){
    ...
    $('#password').blur(function(){
        ...
        if( validator.isPassword(pwd)){
            ...
            showErrTip('passwd', '请输入6-20位数字、字母或常用符号');
        }
    });
    ...
    $('#captcha').blur(function(){
        ...
        if(captcha){
            if(validator.isCode(captcha)){
                ...
                $('#tips-error-captcha').show().html('验证码类型错误');
            }
        }
        else{
        ...
        if(captcha === ''){
            ...
            $.ajax({
                ...
                success: function(msg){
                    if(msg.ret == 1){
                        $('#tips-suc-popup').removeClass('dn');
                        $("#captcha, #password, #repwd, #codeImg").val('');
                        $(".tips-suc, #tips-error-pic-cap, #bk-pic-captcha").hide();
                        $('#btn-change-pwd').removeClass('btn-blue').addClass('btn-grey').prop('disabled', 'true')
                    }
                    else{
