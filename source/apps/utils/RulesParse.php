        ...
        else
        {
            ...
            foreach ($datas as $k =>$v) 
            {
                 if(is_array($v))
                 {
                     ...
                     foreach($v as $keyr => $r)
                     {
                        $arr[$keyr] = htmlspecialchars($r, ENT_NOQUOTES);
                     }
                      $datas[$k] = $arr;
                 }
                 else
                {
                    $datas[$k] = htmlspecialchars($v, ENT_NOQUOTES);
                }
            }
    ...
    protected function _rule($validation, $key, $ruleValue, $ruleMsg, $param)
    {
        ...
        switch($key)
        {
            ...
            case 'length':  // 验证长度
                ...
                $validation->add($param, new \Ucenter\Utils\MyValidator\StrlenValidator(array(
                    'max' => $max,
                    'min' => $min,
                    'message' => $ruleMsg,
                    'code' => $errorCode
            ...
            case 'nums': // 选中个数校验
                $validation->add($param, new \Ucenter\Utils\MyValidator\NumsValidator(array(
            ...
            case 'filetype': // 文件类型合法性校验
                $validation->add($param, new \Ucenter\Utils\MyValidator\FileTypeValidator(array(
    ...
    private function _warnFormat($msgObj, $type='both')
    {
        ...
        {   
            ...
            if(empty($msgArr[$field]))
            {
                switch ($type) {
                    case 'code':
                            $msgArr[$field]['code'] = $this->_errorCode($field);
                    break;
